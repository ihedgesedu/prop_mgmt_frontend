  ...
  try {
    ...
    const expenseLists = await Promise.all(
      properties.value.map(async (property) => {
        try {
          const res = await apiClient.get(`/expenses/${property.id}`);
          let data = res.data;
          if (data && typeof data === 'object' && !Array.isArray(data)) {
            data = data.expenses || data.data || [];
          }
    ...
    expenseRecords.value = expenseLists
      .flat()
      .sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
  } catch (err) {
    console.error('Error loading expenses page:', err);
    error.value = 'Failed to load expense records.';
  } finally {
    loading.value = false;
  }
};
...
const selectProperty = (property: any) => {
  ...
  newExpense.value.property_id = Number(property.id);
...
const addExpense = async () => {
  if (!newExpense.value.property_id) {
    alert('Please select a property before saving expense.');
    return;
  }
  ...
  <div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-500">View and manage all expense records across properties.</p>
      </div>
      <button
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="record in expenseRecords" :key="`${record.id}-${record.property_id}`" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 text-sm text-gray-700">{{ record.category }}</td>
              <td class="px-6 py-4 text-sm text-gray-700">{{ record.vendor }}</td>
              <td class="px-6 py-4 text-sm text-gray-700">{{ record.description }}</td>
              <td class="px-6 py-4 text-sm font-bold text-rose-600 text-right">-${{ formatCurrency(record.amount) }}</td>
            </tr>
            <tr v-if="expenseRecords.length === 0">
              <td colspan="6" class="px-6 py-12 text-center text-gray-400 italic">No expense records found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="isExpenseModalOpen" class="fixed inset-0 z-[120] bg-black/50 backdrop-blur-sm p-4 flex items-center justify-center">
        ...
        <div class="absolute inset-0 bg-white flex">
          ...
          >
            <div class="flex items-center justify-between mb-6">
              <h3 class="text-2xl font-bold text-gray-900">Add Expense Record</h3>
              <button @click="isExpenseModalOpen = false; resetExpenseForm()" class="p-2 hover:bg-gray-100 rounded-full">
                <X class="w-6 h-6 text-gray-400" />
              </button>
            </div>

            <form @submit.prevent="addExpense" class="space-y-5">
              <div class="space-y-2">
              ...
              <div class="space-y-2">
                ...
                <input v-model="newExpense.vendor" required type="text" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
              <div class="space-y-2">
                <label class="text-sm font-semibold text-gray-700">Description</label>
                <textarea v-model="newExpense.description" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none h-24"></textarea>
              </div>
              <div class="flex justify-end gap-3 pt-2">
                <button type="button" @click="isExpenseModalOpen = false; resetExpenseForm()" class="px-4 py-2 border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Save Expense</button>
              </div>
            </form>
          </div>
